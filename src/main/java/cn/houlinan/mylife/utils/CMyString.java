...
public class CMyString {
    ...
    /**
     *
     * Description: 判断指定字符串是否为空，并且是否为数字 <BR>
     *
     * @author wen.junhui
     * @date 2016年1月18日 下午10:23:24
     * @param _string
     * @return 若字符串为空对象（_string==null）或空串（长度为0）且不是数字，则返回true；否则，返回false.
     */
    public static boolean isInteger(String _string) {
        if (isEmpty(_string) || !RegexUtil.isInteger(_string))
            return true;
        return false;
    ...
    public static String getStr(String _strSrc, String _encoding) {
        ...
        try {
            ...
            /*
             * 如上的实现和下面的方法调用的实现是等价的，同样地丢弃了16位字符的高8位。 _strSrc.getBytes(0, _strSrc.length(),
             * byteStr, 0); 之所以这样写，而不是String类型的方法调用（如上），是要明确这种丢弃高8位行为。
             */
     ...
     * 将指定文本内容，格式化为bbs风格的Html文本字符串。
     * <p>
     * 说明：[1]处理折行时，若使用 style="WORD_WRAP:keepall"，则不能将空格转换为 <code>&amp;nbsp;</code>
     * </p>
     * <p>
    ...
    public static String filterForSQL(String _sContent) {
        ...
        /*
         * for( int i=0; i <nLen; i++ ){ char cTemp = srcBuff[i]; switch( cTemp ){ case
         * '\'':{ retBuff.append( "''" ); break; } case '!': case '@': case '#': case
         * '$': case '%': case '^': case '&': case '*': case '(': case ')': case '+':
         * case '|': case '-': case '=': case '\\': case ';': case ':': case '\"': case
         * ',': case '.': case '/': case ' <': case '>': case '?': break; //skip default
         * : retBuff.append( cTemp ); }//case }//end for
         */
    ...
    /**
     * Description: 过滤HTML、JS、SQL字符，对这些特殊字符做转义处理 <BR>
     *
     * @author liu.jian
     * @date 2016年4月8日 下午7:26:34
     * @param _sContent
     *            需处理的字符串
     * @return 处理后的字符串
     */
    public static String filterForAll(String _sContent) {
        String _sFilter = _sContent;
        _sFilter = filterForHTMLValue(_sFilter);
        _sFilter = filterForJs(_sFilter);
        _sFilter = filterForSQL(_sFilter);
        return _sFilter;
    }
    // ==============================================================================
    // 数字转化为字符串
    ...
    public static String cutStr(String _string, int _length) {
         ...
         * nTotalLen += 2; }
         *
         * if( nTotalLen <=_length ) { //字符串本身的长度小于需要截取的长度，直接取该字符串 return _string; }
         *
         * else { //否则进行判断 for( int i=0;i <_length;i++ ) { if( _string.charAt(i)>255 )
         * nTmp += 2; //中文字符长度加2 else nLen += 1; //英文字符长度加1
         *
         * nMaxLen += 1; //记数
         *
         * if( nTmp+nLen==_length ) { return ( _string.substring(0,nMaxLen)+".." ); }
         * if( nTmp+nLen>_length ) { return ( _string.substring(0,nMaxLen-1)+".." ); } }
         * //end for } //end else
         *
         * return _string; //
         */
     ...
     * @return 连接后的字符串
     */
    public static <T> String join(List<T> _arColl, String _sSeparator) {
        // check parameters
        if (_arColl == null)
            return null;
    ...
    /**
     * Description: 验证带分隔符的数字格式字符串 ，验证通过返回true，格式为：23,5,215,10,3 <BR>
     *
     * @author cheng.zhifeng
     * @date 2016年1月26日 下午5:23:26
     * @param _str
     *            待验证的字符串
     * @param _sDelim
     *            分隔符，如果为空则使用默认值逗号
     * @return
     */
    public static boolean isNumberFormatStr(String _str, String _sDelim) {
        if (isEmpty(_str))
            return false;
        ...
        return _str.matches("^[\\d]+(" + _sDelim + "[\\d]+)*[\\d]*$");
    }

    public static void main(String[] args) {
        System.out.println(isNumberFormatStr("4;90", ";"));
    }
...
//    /**
//     *DESC: 将“，”拼接的字符串转成List<Long>  </>
//     *@param:  groupDocIds
//     *@return:  java.util.List<java.lang.Long>
//     *@author hou.linan
//     *@date:  2019/6/14 10:57
//    */
//    public static List<Long> strsToListL(String groupDocIds){
//
//        List<Long> collect = new ArrayList<>();
//
//        try {
//            collect = Arrays.stream(groupDocIds.split(DocConstant.DATAId_SEPARATOR))
//                    .map(Long::valueOf).collect(Collectors.toList());
//
//            return collect ;
//        }catch(Exception e){
//            e.printStackTrace();
//            log.error("将字符串转换成List<Long>出现错误。传入的字符串为：{}" ,groupDocIds);
//            return collect ;
//        }
//
//    }
//    public static List<Integer> strsToListInteger(String groupDocIds){
//
...
//        }catch(Exception e){
//            e.printStackTrace();
//            log.error("将字符串转换成List<Integer>出现错误。传入的字符串为：{}" ,groupDocIds);
//            return collect ;
//        }
//        return collect ;
//    }
